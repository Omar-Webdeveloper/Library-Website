...
{
    public partial class test_on_edit : System.Web.UI.Page
    {
        ...
        {
            string file = Server.MapPath("~/OMAR/App_Data/Reigster.txt");
            string imagePath = "~/Images/22.png";

            if ((FileUpload1.PostedFile != null) && (FileUpload1.PostedFile.ContentLength > 0))
            {
                string folderPath = Server.MapPath("~/OMAR/App_Data/ProfielImages/");
                string fileName = Path.GetFileName(FileUpload1.PostedFile.FileName);
                imagePath = Path.Combine(folderPath, fileName);
                FileUpload1.PostedFile.SaveAs(imagePath);
                ProfileImagePathTextBox.Text = "~/Images/" + fileName;
            }
        }
